...
- **[Beacon](https://github.com/JoshTSeppich/Beacon)** — Maps a room with sound from a phone: it plays an exponential sine sweep, records the reflections, deconvolves an impulse response, and turns the echo peaks into wall distances and a floor plan. One HTML file, DSP written by hand; the FFT, sweep, inverse filter, cross-correlation and room solver are plain functions at the top of the file, because the fastest loop for an acoustics experiment on a phone is editing one file. The finding was physical: one phone can't separate its own speaker from its mic, and iOS echo cancellation eats the sweep, so the design moved to two devices paired through a small relay. Prototype, paused; I never validated the distances against a tape measure.
